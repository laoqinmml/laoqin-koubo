...
- **封面比例按平台固定（2026-09-20 用户确认，勿混用）**：**抖音 = 3:4 竖版**、**小红书 = 3:4 竖版**、**视频号 = 3:4 竖版**（用 `yxer upload --platform 视频号 --usage cover` 上传）；**快手 = 16:9 横屏**、**大号组哔哩哔哩 = 16:9 横屏**（不用 3:4）。组装 payload 前先确认每条视频该平台的封面 key 是不是对的比例——用错比例会直接体现在平台作品展示上，需要删掉重发。
